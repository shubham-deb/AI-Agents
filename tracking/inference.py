...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        for pos in positions:
            ...
            beleifs = self.beliefs[pos]
            allPossible[pos] = emissionModel[dist]*beleifs

        if noisyDistance is None:
            allPossible = util.Counter()
            allPossible[self.getJailPosition()] =0.1
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            ...
            for newPos, prob in dist.items():
                beliefs = self.beliefs[oldPos]
                allPossible[newPos] += prob*beliefs
            # print allPossible[newPos]
            # print newPos,prob
        self.beliefs = allPossible
        #util.raiseNotDefined()
...
class ParticleFilter(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        newDist = []
        newPos = self.particlePos
        for p in newPos:
            # print p
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            newPosDist.normalize()
            newDist.append(util.sample(newPosDist))
        self.particlePos = newDist
        #util.raiseNotDefined()
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        self.particlePos = []
        counter = 0
        totalghosts = self.numGhosts
        legalpos = self.legalPositions
        particles = self.numParticles
        pos = list(itertools.product(legalpos, repeat=totalghosts))
        ...
        for pos in self.particlePos:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] is None:
                    new_pos = self.getParticleWithGhostInJail(new_pos,i)
                else:
                    trueDistance = util.manhattanDistance(new_pos[i], pacmanPosition)
                    new_prob *= emissionModels[i][trueDistance]

            allPossible[new_pos] += new_prob
            # self.particlePos = list(map(lambda x: new_pos if x==pos else x, self.particlePos))
        allPossible.normalize()
        c = 0
        for val in allPossible.values():
            c += val
    ...
    def elapseTime(self, gameState):
        ...
        """
        self.particles = self.particlePos
        newParticles = []
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            newPosDist = util.Counter()
            for counter in range(self.numGhosts):
                # print i in range(self.numGhosts
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle)
                                                         , counter, self.ghostAgents[counter])
                # print util.sample(ghostDist)
                newParticle[counter] = util.sample(newPosDist)
            "*** END YOUR CODE HERE ***"
            newParticles.append(tuple(newParticle))
        self.particles = newParticles
        self.particlePos = self.particles
